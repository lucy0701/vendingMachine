const data = {
    'totalAmount': 0,

    'userCoinCount': { coin1: 10, coin5: 10, coin10: 10, coin50: 10, coin100: 10 },

    'machineCoinCount': { '1': 10, '5': 10, '10': 10, '50': 10, '100': 10 },

    'items': [
            { itemName: "여주155", price: 155, stock: 10, image: "/img/img11.png"},
            { itemName: "코순888", price: 888, stock: 10, image: "/img/img12.png"},
            { itemName: "루미700", price: 700, stock: 10, image: "/img/img09.png"},
            { itemName: "콩20", price: 20, stock: 10, image: "/img/img17.png"},
            { itemName: "재희95", price: 95, stock: 10, image: "/img/img10.png"},
            { itemName: "번18", price: 18, stock: 10, image: "/img/img14.png"},
            { itemName: "승목1000", price: 1000, stock: 10, image: "/img/img15.png"},
            { itemName: "죽순500", price: 500, stock: 10, image: "/img/img16.png"}
    ],

    'myItemList': []
}

export default data;


// export default class Data {
//    constructor(){
//     this.totalAmount = 0;
//     this.userCoinCount = { coin1: 10, coin5: 10, coin10: 10, coin50: 10, coin100: 10 };
//     this.machineCoinCount = { '1': 10, '5': 10, '10': 10, '50': 10, '100': 10 };
//     this.items = [
//         { itemName: "여주155", price: 155, stock: 10, image: "/img/img11.png"},
//         { itemName: "코순888", price: 888, stock: 10, image: "/img/img12.png"},
//         { itemName: "루미700", price: 700, stock: 10, image: "/img/img09.png"},
//         { itemName: "콩20", price: 20, stock: 10, image: "/img/img17.png"},
//         { itemName: "재희95", price: 95, stock: 10, image: "/img/img10.png"},
//         { itemName: "번18", price: 18, stock: 10, image: "/img/img14.png"},
//         { itemName: "승목1000", price: 1000, stock: 10, image: "/img/img15.png"},
//         { itemName: "죽순500", price: 500, stock: 10, image: "/img/img16.png"}
//     ];
//     this.getItemList = [];
//    }
// }
